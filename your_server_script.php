...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['quarter']) && isset($_SESSION['user_name'])) {
    ...
    $sql = "UPDATE users SET quarter = ? WHERE email = ?";
    $stmt = mysqli_stmt_init($conn);
    $prepareStmt = mysqli_stmt_prepare($stmt, $sql);

    if ($prepareStmt) {
        mysqli_stmt_bind_param($stmt, "ss", $quarter, $username); // Use "s" for a string value
